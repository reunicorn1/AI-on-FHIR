...
export default function DataView({ data }: DataViewProps) {
          ...
          <thead className="bg-gray-700 sticky top-0 z-10">
            <tr>
              {patients && patients.length > 0 && Object.keys(patients[0]).map((key) => (
                <th key={key} className="px-6 py-3 text-left text-xs font-medium text-gray-300 uppercase tracking-wider border-b border-gray-600">
            ...
            {patients && patients.map((patient: Patient, index: number) => (
              <tr key={index} className="hover:bg-gray-700 transition-colors">
                {Object.values(patient).map((value, cellIndex) => (
                  <td key={cellIndex} className="px-6 py-4 whitespace-nowrap text-sm text-gray-100 border-b border-gray-600">
    ...
    <div className="space-y-8">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
        {summary && Object.entries(summary).map(([key, value]) => {
          ...
          return (
            <div key={key} className="bg-gray-800 p-6 rounded-lg border border-gray-700 shadow-sm">
      ...
      {summary?.locations && (
        <div>
          <h2 className="text-xl font-semibold text-gray-100 mb-4">Location Distribution</h2>
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            {/* Cities */}
            {summary.locations.cities && (
              <div className="bg-gray-800 p-6 rounded-lg border border-gray-700 shadow-sm">
                <h3 className="text-lg font-semibold text-gray-100 mb-4">Cities</h3>
                  ...
                  {Object.entries(summary.locations.cities)
                    ...
                    .map(([city, count]) => (
                      <div key={city} className="flex justify-between items-center">
                        ...
                        <span className="text-sm font-semibold text-blue-400">{String(count)}</span>
                      </div>
                    ))}
                </div>
              </div>
            ...
            {summary.locations.states && (
              <div className="bg-gray-800 p-6 rounded-lg border border-gray-700 shadow-sm">
                <h3 className="text-lg font-semibold text-gray-100 mb-4">States</h3>
                <div className="space-y-2 max-h-60 overflow-y-auto">
  ...
  const renderGenderChart = () => {
    ...
    if (total === 0) {
      return <div className="text-center py-8 text-gray-400">No gender data available</div>;
    }
    ...
    return (
      <div className="bg-gray-800 p-6 rounded-lg border border-gray-700">
        <h3 className="text-xl font-semibold text-gray-100 mb-6">Gender Distribution</h3>
  ...
  const renderAgeChart = () => {
    ...
    if (maxValue === 0) {
      return <div className="text-center py-8 text-gray-400">No age group data available</div>;
    }

    return (
      <div className="bg-gray-800 p-6 rounded-lg border border-gray-700">
        <h3 className="text-xl font-semibold text-gray-100 mb-6">Age Group Distribution</h3>
        <div className="space-y-4">
          {Object.entries(ageData).map(([ageGroup, count]) => (
            <div key={ageGroup} className="flex items-center">
              <div className="w-20 text-sm font-medium text-gray-300 mr-4">
                {ageGroup}
              </div>
  ...
  return (
    <div className="w-full max-w-6xl mx-auto h-[400px] bg-gray-900 rounded-lg overflow-hidden border border-gray-700 shadow-lg flex">
      {/* Sidebar */}
      <div className="w-48 bg-gray-800 border-r border-gray-700 flex-shrink-0">
        <div className="p-4">
          <h2 className="text-lg font-semibold text-gray-100 mb-4">Data Views</h2>
